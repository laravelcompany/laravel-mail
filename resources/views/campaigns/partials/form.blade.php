<x-laravel-mail.text-field name="name" :label="__('Campaign Name')" :value="$campaign->name ?? old('name')" />
<x-laravel-mail.text-field name="subject" :label="__('Email Subject')" :value="$campaign->subject ?? old('subject')" />
<x-laravel-mail.text-field name="from_name" :label="__('From Name')" :value="$campaign->from_name ?? old('from_name')" />
<x-laravel-mail.text-field name="from_email" :label="__('From Email')" type="email" :value="$campaign->from_email ?? old('from_email')" />
...
<x-laravel-mail.textarea-field name="content" :label="__('Content')">{{ $campaign->content ?? old('content') }}</x-laravel-mail.textarea-field>
        ...
        $(function () {
            const smtp = {{
                $emailServices->filter(function ($service) {
                    return $service->type_id === \LaravelCOmpany\Mail\Models\EmailServiceType::SMTP;
                })
                ->pluck('id')
            }};
